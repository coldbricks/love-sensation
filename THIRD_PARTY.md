...
The supplemental covered-body image classifier uses Google's [SigLIP 2 SO400M patch16 512 model](https://huggingface.co/google/siglip2-so400m-patch16-512), pinned at revision `ceea1cba8130d8271436da4828633198c176a775`. Its `model.safetensors` is 4,546,331,880 bytes, SHA-256 `a621bd212e1b3329b428595f9693217e19587afe826adf3e5c241a16392e8973`. Configuration and tokenizer files are also pinned and hash-checked in the downloader. Transformers loads the verified local files with remote code disabled. CUDA FP16 vision execution is verified at runtime; resize and normalization also run on that device. The application uses fixed text comparisons for image-level presence, not a region detector or pose model. No private media or private training examples are distributed with the model or application.
